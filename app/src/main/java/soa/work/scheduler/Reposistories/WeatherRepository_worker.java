...
public class WeatherRepository_worker {
    private static MutableLiveData<WeatherModel> data = new MutableLiveData<>();

    private ApiService apiService;
    private static Call<WeatherModel> call;

    public WeatherRepository_worker(){
        apiService = WeatherClient.getApiService();
        loadData();
    }

    private void loadData() {
        SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(WorkersActivity.activity);
        String weatherCity = sharedPreferences.getString(
                WorkersActivity.activity.getString(R.string.weather_city_key),
                WorkersActivity.activity.getString(R.string.weather_city_default)
        );

        boolean detectCityAutomatically = sharedPreferences.getBoolean(
                WorkersActivity.activity.getString(R.string.detect_city_automatically_key),
                Boolean.parseBoolean(WorkersActivity.activity.getString(R.string.detect_city_automatically_default))
        );
        String city;
